...
struct socks5_request *socks5_read_request (struct tcpConnection *client)
{
  ...
  if (length > 0)
      ...
      if (request->address_type == ATYP_DOMAIN)
	{
	  ...
	  request->bind_port.bytes[0] =
	    request->bind_address.domain.name[name_size];
	  request->bind_port.bytes[1] =
	    request->bind_address.domain.name[name_size + 1];
...
void socks5_auth_print (struct socks5_auth *auth_request)
{
  int i;
...
void socks5_request_print (struct socks5_request *request)
{
  printf ("version: %d\n", request->version);
  ...
  switch (request->address_type)
    ...
    case ATYP_IPV4:
      printf ("%d.%d.%d.%d\n",
	      ...
	      request->bind_address.ipv4[2], request->bind_address.ipv4[3]);
      break;
    case ATYP_IPV6:
      {
	int i;
